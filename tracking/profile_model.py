...
def parse_args():
    ...
    # for train
    parser.add_argument('--script', type=str, default='letrack', choices=['letrack'],
                        help='training script name')
    parser.add_argument('--config', type=str, default='baseline_WOCE', help='yaml configure file name')
    args = parser.parse_args()

    return args


def evaluate_letrack(model, template, search):
    '''Speed Test'''
    model_ = copy.deepcopy(model)
    macs1, params1 = profile(model, inputs=(template, search,True,True,True,True),
    ...
    with torch.no_grad():
        # overall
        for i in range(T_w):
            _ = model_(template, search,profile=True)
        start = time.time()
        for i in range(T_t):
            _ = model_(template, search,profile=True)
        torch.cuda.synchronize()
...
if __name__ == "__main__":
    ...
    yaml_fname = '/home/data/xcc27/ASTrack_MAXinter300/experiments/letrack/baseline_WOCE.yaml'
    config_module.update_config_from_file(yaml_fname)
    ...
    if args.script == "letrack":
        model_module = importlib.import_module('lib.models')
        model_constructor = model_module.build_letrack
        model = model_constructor(cfg, training=False)
        # get the template and search
        template = torch.randn(bs, 64, 192)
        ...
        evaluate_letrack(model, template, search)
