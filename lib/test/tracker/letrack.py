import math
import numpy as np
from lib.models.letrack import build_letrack
...
class LETrack(BaseTracker):
    def __init__(self, params, data):
        super(LETrack, self).__init__(params)
        network = build_letrack(params.cfg, training=False)
    ...
    def initialize(self, image, info: dict):
        ...
        self.hlist = torch.tensor([])

        with torch.no_grad():
            # self.z_dict1 = template
            self.memory_frames = [template.tensors]
            # template_list = self.memory_frames.copy()
            # self.z , _ = self.network.forward_test(template=template_list, search=None, mode='z', ce_template_mask=None)
        # self.memory_masks = []
    ...
    def track(self, image, info: dict = None):
        ...
        with torch.no_grad():
            out_dict, aux_dict = self.network.forward_test(self.memory_frames, search=[search.tensors], mode='zx', ce_template_mask=None, hlist = self.hlist )
            # out_dict, aux_dict = self.network.forward_zx(self.memory_frames, search=[search.tensors], hlist = self.hlist )
