...
from lib.train.actors import LETrackActor
# for import modules
import importlib
...
def run(settings):
    ...
    # Create network
    if settings.script_name == "letrack":
        net = build_letrack(cfg)
    else:
    ...
    if settings.script_name == "letrack":
        ...
        actor = LETrackActor(net=net, objective=objective, loss_weight=loss_weight, settings=settings, cfg=cfg)
    else:
        raise ValueError("illegal script name")
