...
from lib.models.letrack.utils import combine_tokens, recover_tokens
...
class BaseBackbone(nn.Module):
    ...
    def forward(self, z, x, **kwargs):
        ...
        # else:
        if kwargs['mode'] == 'z':
            z, aux_dict = self.forward_z(z)
            return z, aux_dict
        elif kwargs['mode'] == 'zx':
            x, aux_dict = self.forward_zx(z,x,kwargs['hlist'])
            return x, aux_dict
    ...
    def forward_z(self, z):
        ...
        aux_dict = {"attn": None,"hlist": None}
        
        return self.norm(z), aux_dict
    


    def forward_zx(self, z, x, hlist):
        ...
        if hlist.numel() == 0:
            ...
            zh = torch.cat((z, hlist), dim=1)
            
        x = torch.cat((zh, x),dim=1)

        x = self.pos_drop(x)
        # len_x = x.shape[1]
        for i in range(0,12):
            if i < 11:
                x = self.blocks[i](x, return_attention=False)
            else:
                x, attn_s2ts = self.blocks[i](x, return_attention=True)            

                    
        aux_dict = {"attn": None,"attn_s2ts":attn_s2ts}
    ...
    def forward_zx_test(self, z, x, hlist):
